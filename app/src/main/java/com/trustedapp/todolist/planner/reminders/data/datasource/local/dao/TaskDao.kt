...
interface TaskDao {
    ...
    @Transaction
    @Query("select * from TaskEntity where (dueDate == :dayString or (calendar / 1000 = :start / 1000)) and isDone = 0")
    fun getTaskInDay(dayString: String, start: Long): List<TaskShort>

    @Transaction
    @Query("select * from TaskEntity where (dueDate == :dayString or (calendar / 1000 = :start / 1000))")
    fun getTaskInDayAll(dayString: String, start: Long): List<TaskShort>
